...
function SchedulerRow({ database, sessionId, index, array, setUpdateSession, highlightedTimeslots }) {
  ...
  return (
    <tr key={"schedule-row-" + index} className={(justBookedSession == sessionId ? "highlighted-session-row" : "") + (index < array.length - 1 ? (sessionId.substring(0, 13) != array[index + 1].substring(0, 13) ? " day-separator" : "") : "")}>
